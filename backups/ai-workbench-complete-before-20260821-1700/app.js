...
function parseAIAnalysis(response){
  ...
  return {summary:String(parsed.summary||"AI 已完成今日判断"),priorities:priorities.slice(0,5).map(x=>({taskId:x.taskId,title:String(x.title||"未命名任务"),level:String(x.level||"中"),reason:String(x.reason||"未提供判断原因"),nextAction:String(x.nextAction||"请确认下一步行动")})),risks:risks.slice(0,5),suggestions:suggestions.slice(0,5),confidence:Number(parsed.confidence)||0.5,generatedAt:new Date().toISOString(),source:"api"};
}
