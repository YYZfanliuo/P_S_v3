...
function setAIConfig(cfg){ data.__aiConfig = { ...getAIConfig(), ...cfg }; store.save(); }
...
function aiConfigHeaders(cfg){
  ...
  else if(cfg.authType==="apiKey"&&cfg.apiKey) headers["x-api-key"]=cfg.apiKey;
  if(cfg.customHeaders){
    try{ Object.assign(headers,JSON.parse(cfg.customHeaders)); }catch(e){}
  }
  return headers;
}
async function aiBridgeRequest(cfg,messages){
  ...
  try{
    const res=await fetch(`${AI_BRIDGE_ENDPOINT}/api/ai/chat`,{method:"POST",headers:{"Content-Type":"application/json"},signal:controller.signal,body:JSON.stringify({config:cfg,messages})});
...
function aiResponseText(response){
  const content=response?.choices?.[0]?.message?.content ?? response?.choices?.[0]?.text ?? response?.output_text ?? response?.content ?? response?.text;
...
async function plannerAIAnalyzeToday(){
  ...
  try{
    ...
    const response=await aiBridgeRequest(cfg,[{role:"system",content:"你是一个严谨、可解释、只提供建议的个人计划判断助手。"},{role:"user",content:prompt}]);
    const analysis=parseAIAnalysis(response);
    data.__planner={...(data.__planner||{}),aiToday:{analysis,decisions:data.__planner?.aiToday?.decisions||{}}};
...
function renderSettings() {
  $("#screen").innerHTML = `<div class="header"><div><h2>设置</h2><p>个性化你的工作台</p></div><div class="spacer"></div><span class="date-chip">${icon("calendar",14)} ${dateStr()}</span></div>
    ...
    <div class="card ai-config-card" style="padding:20px;display:flex;flex-direction:column;gap:15px;">
      <div class="ai-config-intro"><div><div style="font-weight:700;font-size:14px;">AI 接口配置</div><div style="font-size:12px;color:var(--text-secondary);margin-top:3px;">仅保存在本机浏览器中，默认使用常见的兼容格式。</div></div><span class="ai-config-state" id="ai-config-state">${getAIConfig().endpoint?'已配置':'未配置'}</span></div>
      <div class="field"><label>服务名称</label><input id="f-ai-name" value="${attr(getAIConfig().name||'')}" placeholder="例如：我的本地模型"/></div>
      <div class="field"><label>接口地址</label><input id="f-ai-endpoint" type="url" value="${attr(getAIConfig().endpoint||'')}" placeholder="例如：http://127.0.0.1:8000/v1/chat/completions"/></div>
      <div class="field"><label>模型名称</label><input id="f-ai-model" value="${attr(getAIConfig().model||'')}" placeholder="例如：my-model"/></div>
      <div class="field"><label>认证信息 <span style="font-size:11px;color:var(--text-tertiary);font-weight:500;">可留空</span></label><div style="display:flex;gap:8px;align-items:center;"><select id="f-ai-auth" style="width:130px;"><option value="bearer" ${getAIConfig().authType!=='apiKey'?'selected':''}>Bearer Token</option><option value="apiKey" ${getAIConfig().authType==='apiKey'?'selected':''}>API Key</option><option value="none" ${getAIConfig().authType==='none'?'selected':''}>无需认证</option></select><input id="f-ai-key" type="password" value="${attr(getAIConfig().apiKey||'')}" placeholder="填写后保存在本机" style="flex:1;"/></div></div>
      <details class="ai-config-advanced"><summary>高级设置</summary><div class="ai-config-advanced-body"><div class="planner-row"><div class="field"><label>超时时间（毫秒）</label><input id="f-ai-timeout" type="number" min="1000" step="1000" value="${Number(getAIConfig().timeout)||30000}"/></div><div class="field"><label>最大输出长度</label><input id="f-ai-max-tokens" type="number" min="1" step="1" value="${Number(getAIConfig().maxTokens)||1200}"/></div></div><div class="field"><label>自定义请求头（JSON，可选）</label><textarea id="f-ai-headers" rows="3" placeholder='例如：{"X-Custom":"value"}'>${esc(getAIConfig().customHeaders||'')}</textarea></div></div></details>
  ...
  // ---- 自定义 AI 模型 ----
  const readAIForm=()=>({name:$("#f-ai-name").value.trim(),endpoint:$("#f-ai-endpoint").value.trim(),model:$("#f-ai-model").value.trim(),authType:$("#f-ai-auth").value,apiKey:$("#f-ai-key").value,timeout:Number($("#f-ai-timeout").value)||30000,maxTokens:Number($("#f-ai-max-tokens").value)||1200,customHeaders:$("#f-ai-headers").value.trim()});
  ...
  $("#btn-ai-save").onclick=()=>{ const cfg=readAIForm(); if(!cfg.endpoint||!cfg.model){ aiMessage("请至少填写接口地址和模型名称"); return; } setAIConfig({...cfg,lastTest:null}); $("#ai-config-state").textContent="已配置"; $("#btn-ai-clear").disabled=false; aiMessage("配置已保存在本机",true); toast("AI 配置已保存"); };
