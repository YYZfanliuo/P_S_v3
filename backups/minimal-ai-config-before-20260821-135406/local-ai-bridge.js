...
function buildHeaders(config) {
  ...
  if (config.customHeaders) {
    try {
      const extra = JSON.parse(config.customHeaders);
      if (!extra || typeof extra !== "object" || Array.isArray(extra)) throw new Error("请求头必须是 JSON 对象");
      Object.entries(extra).forEach(([key, value]) => { headers[key] = String(value); });
    } catch (err) {
      throw new Error("自定义请求头不是有效的 JSON");
    }
  }
  return headers;
}
