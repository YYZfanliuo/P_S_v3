...
async function testAIConnection(cfg){
  ...
  try{
    const res=await fetch(cfg.endpoint.replace(/\/$/,""),{method:"POST",headers:aiConfigHeaders(cfg),signal:controller.signal,body:JSON.stringify({model:cfg.model,messages:[{role:"user",content:"请只回复：连接测试成功"}],temperature:0,max_tokens:20})});
    if(!res.ok){ const text=await res.text(); throw new Error(`接口返回 HTTP ${res.status}${text?`：${text.slice(0,160)}`:""}`); }
...
function plannerAIAnalyzeToday(){
  ...
  persist(); renderExecution(); toast('已生成今日 AI 判断预览');
}
