...
function parseAIPlan(response){
  ...
  try{ parsed=JSON.parse(cleaned); }catch(e){
    ...
    try{ parsed=JSON.parse(match[0]); }catch(err){ throw new Error("AI 计划 JSON 格式不完整"); }
  }
...
function plannerRecordAIAction(action, details={}){
  ...
  actions.unshift({id:plannerUid("ai-action"),at:new Date().toISOString(),action,view,details});
  data.__planner={...p,aiActions:actions.slice(0,200)};
}
...
async function plannerAIGeneratePlan(input){
  ...
  if(!cfg.endpoint||!cfg.model) return {plan:null,reason:"未配置真实 AI"};
  const modeLabels={simple:"简易模式",professional:"专业模式",smart:"智能模式"};
  const workspace=aiWorkspacePayload();
  const prompt=`你是个人工作台的计划生成引擎。当前模式：${modeLabels[input.mode]||input.mode}。请根据用户目标、已有工作台数据和输入信息，生成可执行任务计划。只输出合法 JSON，不要 Markdown、解释或思考过程。最多 10 个任务；每个任务包含 title、priority(P0/P1/P2/P3)、estimatedMinutes、reason、nextAction、successCriteria、dependencies。固定结构：{"summary":"不超过100字","milestoneTitle":"里程碑名称","deliverableTitle":"交付物名称","tasks":[{"title":"任务","priority":"P0","estimatedMinutes":30,"reason":"原因","nextAction":"下一步","successCriteria":"完成标准","dependencies":[]}],"assumptions":[],"questions":[],"confidence":0到1之间的数字}。不要虚构用户未提供的事实；缺失信息放入 assumptions 或 questions。当前输入：${JSON.stringify({mode:input.mode,title:input.title,startDate:input.startDate,deadline:input.deadline,current:input.current,available:input.available,scope:input.scope,files:data.__planner?.files||[],workspace})}`;
  const messages=[{role:"system",content:"你是严谨的个人工作台计划生成引擎，只返回最终 JSON。"},{role:"user",content:prompt}];
  let response;
  try{ response=await aiBridgeRequest(cfg,messages,{responseFormat:{type:"json_object"},maxTokens:3600}); }
  catch(firstError){ devLog("warn","AI 计划生成","结构化请求失败，改用普通请求",{message:firstError.message}); response=await aiBridgeRequest(cfg,messages,{maxTokens:3600}); }
  const plan=parseAIPlan(response);
  plannerRecordAIAction("generate-plan",{mode:input.mode,source:"api",summary:plan.summary,taskCount:plan.tasks.length,confidence:plan.confidence});
  devLog("info","AI 计划生成","计划解析成功",{mode:input.mode,summary:plan.summary,taskCount:plan.tasks.length,confidence:plan.confidence});
  return {plan};
}
...
function plannerTaskStatus(task,status,delayReason,delayNote){
  ...
  let replan=null;
  if(item.plannerGoalId){ plannerRefreshProgress(item.plannerGoalId); replan=plannerReplanRemaining(item.plannerGoalId); plannerRecordAIAction("execution-update",{taskId:item.id,status:next,replannedBlocks:replan?.updated||0,delayReason:item.delayReason||""}); }
  persist();
...
function renderPlanner(){
  ...
  const questionRows=d?.aiQuestions?.length?`<div class="planner-questions"><b>AI 仍需要确认</b>${d.aiQuestions.map(q=>`<span>${esc(q)}</span>`).join("")}</div>`:"";
  ...
  $("#planner-generate").onclick=async()=>{ const mode=plannerState().mode; const perms=plannerState().permissions; const input={mode,title:$("#planner-title").value,startDate:$("#planner-start").value,deadline:$("#planner-deadline").value,current:$("#planner-current").value,available:$("#planner-available").value,scope:$("#planner-scope").value}; const button=$("#planner-generate"); if(button){ button.disabled=true; button.textContent="AI 正在生成任务…"; } try{ const result=await plannerAIGeneratePlan(input); const draft=plannerBuildDraft({...input,aiPlan:result.plan||null}); data.__planner={...(data.__planner||{}),permissions:{...perms,writeTasks:$("#planner-perm-tasks").checked,writeSchedule:$("#planner-perm-schedule").checked},draft,aiPlanLastRun:{at:new Date().toISOString(),mode,source:result.plan?"api":"local-rule",reason:result.reason||""}}; plannerSaveGoalRecord(draft,"active"); data.__planner.draft=draft; persist(); renderPlanner(); toast(result.plan?`AI 已生成 ${draft.tasks.length} 项任务，请核对计划`:`真实 AI 不可用，已生成本地计划草案`); }catch(err){ devLog("error","AI 计划生成","真实 AI 失败，使用本地规则草案",{message:err.message,name:err.name}); const draft=plannerBuildDraft(input); data.__planner={...(data.__planner||{}),permissions:{...perms,writeTasks:$("#planner-perm-tasks").checked,writeSchedule:$("#planner-perm-schedule").checked},draft,aiPlanLastRun:{at:new Date().toISOString(),mode,source:"local-fallback",reason:err.message}}; plannerSaveGoalRecord(draft,"active"); data.__planner.draft=draft; persist(); renderPlanner(); toast(`AI 计划生成失败，已使用本地草案：${err.message}`); } };
...
async function plannerAIAnalyzeToday(){
  ...
  try{
    const prompt=`你是个人工作台的判断助手。只输出合法 JSON，不要 Markdown、解释或思考过程。最多输出 3 条 priorities、3 条 risks、3 条 suggestions；每个 reason 和 nextAction 各不超过 60 字；summary 不超过 80 字。必须完整闭合 JSON，不能重复字段，不能截断。结构：{"summary":"简短总结","priorities":[{"taskId":"任务ID","title":"任务标题","level":"高/中/低","reason":"判断原因","nextAction":"具体下一步"}],"risks":["风险"],"suggestions":[{"taskId":"任务ID","title":"建议","reason":"原因"}],"confidence":0到1之间的数字}。你只能分析和提出建议，不能要求系统自动修改计划。以下是工作台数据：${JSON.stringify(aiWorkspacePayload())}`;
    const messages=[{role:"system",content:"你是严谨、可解释、只提供建议的个人计划判断助手。只输出最终合法 JSON，不要输出思考过程。"},{role:"user",content:prompt}];
    let response;
    try{
      response=await aiBridgeRequest(cfg,messages,{responseFormat:{type:"json_object"},maxTokens:3600});
    }catch(firstError){
      devLog("warn","AI 今日判断","结构化请求失败，改用普通请求",{message:firstError.message});
      response=await aiBridgeRequest(cfg,messages,{maxTokens:3600});
    }
    let analysis;
    try{ analysis=parseAIAnalysis(response); devLog("info","AI 今日判断","首次响应解析成功",{analysis}); }
    catch(parseError){
      const rawText=aiResponseText(response);
      devLog("warn","AI 今日判断","首次响应解析失败，准备二次整理",{message:parseError.message,finishReason:aiFinishReason(response),rawText});
      if(!rawText) throw parseError;
      const repairPrompt=`请将下面的 AI 分析内容整理为严格 JSON。不要重新分析，不要补充新判断，只做格式转换。若原文被截断或结构不完整，直接返回空数组和说明，不要猜测。只返回 JSON，不要 Markdown。固定格式：{"summary":"简短总结","priorities":[],"risks":[],"suggestions":[],"confidence":0到1之间的数字}。原始内容：${rawText}`;
      const repairMessages=[{role:"system",content:"你只负责格式修复。不要思考、不要解释、不要重新分析，只输出最终合法 JSON。"},{role:"user",content:repairPrompt}];
      const repaired=await aiBridgeRequest(cfg,repairMessages,{responseFormat:{type:"json_object"},maxTokens:1800}).catch(()=>aiBridgeRequest(cfg,repairMessages,{maxTokens:1800}));
      analysis=parseAIAnalysis(repaired);
      devLog("info","AI 今日判断","二次整理成功",{analysis});
    }
    analysis.source="api";
    plannerRecordAIAction("today-analysis",{source:"api",taskCount:analysis.priorities?.length||0,confidence:analysis.confidence});
    data.__planner={...(data.__planner||{}),aiToday:{analysis,decisions:data.__planner?.aiToday?.decisions||{}}};
    persist(); renderExecution(); toast('已生成真实 AI 判断');
  }catch(err){
    devLog("error","AI 今日判断","接口失败后使用本地兜底",{message:err.message,name:err.name});
