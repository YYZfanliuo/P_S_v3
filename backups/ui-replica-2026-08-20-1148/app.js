...
function dailyGridSegmentHTML(event, hour, segmentStart, segmentEnd, startTotal, endTotal){
  const color = event.color || (event.source === "timetable" ? "#6f9f84" : "#8f83a8");
  const txt = contrastText(color);
  const titleText = event.title || event.courseName || "无标题";
  const timeText = `${event.startTime || "00:00"}–${event.endTime || "00:00"}`;
  const segmentMinutes = Math.max(1, segmentEnd - segmentStart);
  const left = (segmentStart / 60) * 100;
  const width = ((segmentEnd - segmentStart) / 60) * 100;
  const top = ((segmentStart - hour * 60) / 60) * 100;
  const height = Math.max(8, ((segmentEnd - segmentStart) / 60) * 100);
  const compact = segmentMinutes < 36 || width < 18;
  const micro = segmentMinutes < 20 || width < 11;
  const sourceLabel = event.source === "timetable" ? "课程" : "日程";
  const fullLabel = `${sourceLabel}：${titleText}，${timeText}`;
  const shortLabel = titleText.slice(0, 1) || sourceLabel.slice(0, 1);
  return `<button class="daygrid-event ${event.source === "timetable" ? "is-course" : "is-schedule"}${compact ? " is-compact" : ""}${micro ? " is-micro" : ""}" style="top:${top}px;height:${height}px;left:${left}%;width:${width}%;background:${color};color:${txt}" data-timeline-id="${event.id}" data-tooltip="${attr(fullLabel)}" aria-label="${attr(fullLabel)}" title="${attr(fullLabel)}">
    <span class="daygrid-event-source">${sourceLabel}</span><strong>${titleText ? esc(titleText) : esc(shortLabel)}</strong><small>${esc(timeText)}</small><span class="daygrid-event-short" aria-hidden="true">${esc(shortLabel)}</span>
  </button>`;
}
function renderDailyMinuteGrid(dateValue, events, mode = "schedule"){
  const rows = Array.from({length: 24}, (_, hour) => {
    ...
    events.forEach(event => {
      ...
      if (segmentEnd > segmentStart) segments.push(dailyGridSegmentHTML(event, hour, segmentStart, segmentEnd, start, end));
    });
  ...
  return `<div class="daygrid-shell" data-timeline-mode="${mode}">
    <div class="daygrid-corner">时间</div><div class="daygrid-header"><span>小时</span><div class="daygrid-minute-head">${["00","10","20","30","40","50"].map(minute => `<span>${minute}</span>`).join("")}</div></div>
...
function renderTimelineDayView(dateValue, events, mode = "schedule"){
  ...
  ${renderDailyMinuteGrid(dateValue, events, mode)}`;
}
...
function timelineWeekDates(offset = 0){
  const now = new Date(); const dow = (now.getDay() + 6) % 7;
  const monday = new Date(now); monday.setDate(now.getDate() - dow + offset * 7); monday.setHours(12,0,0,0);
...
function renderUnifiedScheduleWeek(){
  const days = timelineWeekDates(scheduleWeekOffset);
...
function plannerExistingBlocks(date){
  ...
  (data.timetable||[]).filter(x=>x.dayOfWeek===weekday).forEach(x=>blocks.push({start:plannerTimeMinutes(x.startTime),end:plannerTimeMinutes(x.endTime),title:x.courseName||x.title||"课程",source:"课程"}));
  return blocks.filter(x=>x.start>=0&&x.end>x.start);
}
function plannerBuildSchedule(tasks,startDate,deadline,availableMinutes){
  const blocks=[]; const conflicts=[]; const dayMinutes=Math.max(15,availableMinutes||120);
  let taskIndex=0, cursorDate=startDate, days=0;
  while(taskIndex<tasks.length && days<370){
    if(deadline && cursorDate>deadline) break;
    const existing=plannerExistingBlocks(cursorDate);
    let used=0;
    // 默认把可用时段放在晚间；遇到课程/既有日程会跳过冲突。
    let cursor=19*60;
    for(let guard=0;guard<48&&used<dayMinutes&&taskIndex<tasks.length;guard++){
      ...
      if(hit){ conflicts.push({date:cursorDate,task:task.title,with:hit.title,source:hit.source}); cursor=Math.max(cursor,hit.end); continue; }
      if(end>23*60+59){ break; }
      blocks.push({date:cursorDate,startTime:plannerTimeText(start),endTime:plannerTimeText(end),title:task.title,taskIndex,estimatedMinutes:chunk});
  ...
  return {blocks,conflicts,capacityDays:days};
}
...
function plannerBuildDraft(input){
  ...
  if(schedule.conflicts.length) assumptions.push(`发现 ${schedule.conflicts.length} 个课程或已有日程冲突，写入前需要确认`);
  return {id:Date.now(),mode,title,deadline,current,available,startDate,scope:input.scope||"long_term",availableMinutes,totalMinutes,capacityMinutes,availableDays,createdAt:new Date().toISOString(),status:"draft",assumptions,tasks,scheduleBlocks:schedule.blocks,conflicts:schedule.conflicts,goal:{title,deadline,current},milestones:[{id:`${Date.now()}-m1`,title:"完成目标交付",deadline}],deliverables:[]};
}
function plannerApplyDraft(){
  ...
  if(p.permissions.writeTasks){
    data.todo=data.todo||[];
    d.tasks.forEach((t,i)=>data.todo.unshift({id:now+i,title:t.title,priority:t.priority,done:false,note:`来源：${d.title}；预计 ${t.estimatedMinutes} 分钟；计划范围：${d.startDate||isoToday()} 至 ${d.deadline||"未设置"}`,plannerGoalId:d.id,planScope:"long_term"}));
  }
  if(p.permissions.writeSchedule&&d.scheduleBlocks?.length){
    data.schedule=data.schedule||[];
    d.scheduleBlocks.forEach((b,i)=>data.schedule.push({id:now+1000+i,title:b.title,date:b.date,startTime:b.startTime,endTime:b.endTime,relatedModule:"todo",relatedItemId:"",color:"#8070b0",note:`来源：${d.title}；规划时间块`,plannerGoalId:d.id,planScope:"today"}));
  }
  data.__planner={...(data.__planner||{}),draft:{...d,status:"applied",appliedAt:new Date().toISOString()},goal:d.goal,milestones:d.milestones,deliverables:d.deliverables};
  persist();
  toast(p.permissions.writeSchedule?"计划已写入任务和日程预览":"计划已写入任务；日程仍未写入");
}
...
function renderPlanner(){
  ...
  const taskRows=d?d.tasks.map((t,i)=>'<div class="planner-task"><span class="planner-task-index">'+(i+1)+'</span><div><b>'+esc(t.title)+'</b><small>'+esc(t.reason)+' · '+t.estimatedMinutes+' 分钟</small></div><span class="priority-pill">'+t.priority+'</span></div>').join(""):"";
  const scheduleRows=d?.scheduleBlocks?.length?d.scheduleBlocks.map(x=>`<div class="planner-schedule-row"><span>${esc(x.date)} ${plannerWeekday(x.date)}</span><b>${esc(x.startTime)}-${esc(x.endTime)}</b><span>${esc(x.title)}</span></div>`).join(""):"<div class=\"planner-muted\">生成后显示建议时间块</div>";
  ...
  const draftHtml=d?`<div class="planner-draft"><div class="planner-draft-head"><div><span class="eyebrow">计划草案 · ${plannerModeLabel(d.mode)} · ${d.scope==="today"?"今日":d.scope==="short_term"?"近期":"长期"}</span><h3>${esc(d.title)}</h3></div><span class="planner-status">${d.status==="applied"?"已写入":"待确认"}</span></div><div class="planner-meta"><span>范围：${esc(d.startDate||isoToday())} 至 ${esc(d.deadline||"未设置")}</span><span>容量：${d.capacityMinutes||0} / ${d.totalMinutes||0} 分钟</span><span>任务：${d.tasks.length} 项</span></div>${assumptions}${conflictRows}<div class="planner-task-list">${taskRows}</div><div class="sec-title">建议时间块</div><div class="planner-schedule-list">${scheduleRows}</div><div class="planner-actions"><button class="btn" id="planner-apply" ${d.status==="applied"?"disabled":""}>先备份，再按权限写入</button><button class="btn ghost" id="planner-discard">清除草案</button></div></div>`:'<div class="planner-empty">输入一个目标后，计划草案会先出现在这里。当前版本不会自动修改任何数据。</div>';
  ...
  $("#planner-generate").onclick=()=>{ const mode=plannerState().mode; const perms=plannerState().permissions; data.__planner={...(data.__planner||{}),permissions:{...perms,writeTasks:$("#planner-perm-tasks").checked,writeSchedule:$("#planner-perm-schedule").checked},draft:plannerBuildDraft({mode,title:$("#planner-title").value,startDate:$("#planner-start").value,deadline:$("#planner-deadline").value,current:$("#planner-current").value,available:$("#planner-available").value,scope:$("#planner-scope").value})}; store.save(); renderPlanner(); toast(mode==="smart"?"已生成追问后的本地草案，请核对缺失信息":"已生成草案，请检查假设和影响范围"); };
...
function renderSchedule() {
  ...
  if (scheduleViewMode === "daily") {
    const today = scheduleSelectedDate || timelineDateOffset(scheduleDayOffset);
    content = renderTimelineDayView(today, scheduleEventsForDate(today), "schedule");
  ...
  if (scheduleViewMode === "daily") {
    ...
    if (prevDay) prevDay.onclick = () => { scheduleDayOffset--; scheduleSelectedDate = null; render(); };
    if (nextDay) nextDay.onclick = () => { scheduleDayOffset++; scheduleSelectedDate = null; render(); };
    if (todayDay) todayDay.onclick = () => { scheduleDayOffset = 0; scheduleSelectedDate = null; render(); };
...
function renderTimetableDayView(allClasses){
  const dateValue = timetableSelectedDate || isoToday();
  const events = scheduleEventsForDate(dateValue).filter(e => e.source === "timetable");
  return renderTimelineDayView(dateValue, events, "timetable");
}
function renderTimetableWeekView(allClasses){
  const days = timelineWeekDates(timetableWeekOffset);
  return renderUnifiedWeeklyTimeline(days, date => scheduleEventsForDate(date).filter(e => e.source === "timetable"), "timetable");
}
...
function renderModule(key){
  ...
  const timetableView = m.type === "timetable" ? `<div class="seg timetable-switch" id="timetable-view-switcher"><div class="opt ${timetableViewMode === 'daily' ? 'on' : ''}" data-v="daily">日视图</div><div class="opt ${timetableViewMode === 'weekly' ? 'on' : ''}" data-v="weekly">周视图</div></div>${timetableViewMode === "weekly" ? renderTimetableWeekView(all) : renderTimetableDayView(all)}` : "";
  ...
  if(m.type === "timetable"){
    ...
    const prevDay = $("#timeline-prev-day"), nextDay = $("#timeline-next-day"), todayDay = $("#timeline-today");
    if(prevDay) prevDay.onclick = () => { timetableSelectedDate = timelineDateOffset(-1); renderModule(key); };
    if(nextDay) nextDay.onclick = () => { timetableSelectedDate = timelineDateOffset(1); renderModule(key); };
    if(todayDay) todayDay.onclick = () => { timetableSelectedDate = null; renderModule(key); };
...
function buildNav(){
  ...
  html.push(`<div class="nav-sep">规划</div>`, `<div class="navi" data-go="planner">${icon("target",19)}计划中心</div>`);
