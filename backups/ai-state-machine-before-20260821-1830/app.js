...
async function plannerAIGeneratePlan(input){
  ...
  if(!cfg.endpoint||!cfg.model){ plannerRecordAIAction("generate-plan-local",{mode:input.mode,source:"local-rule",reason:"未配置真实 AI"}); return {plan:null,reason:"未配置真实 AI"}; }
  ...
  devLog("info","AI 计划生成","计划解析成功",{mode:input.mode,summary:plan.summary,taskCount:plan.tasks.length,confidence:plan.confidence,responseMeta:aiResponseMeta(response)});
  return {plan};
}
