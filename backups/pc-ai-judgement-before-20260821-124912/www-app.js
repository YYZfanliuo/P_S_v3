...
function executionPanelHTML(){
  ...
  return `<div class="execution-panel"><div class="execution-hero"><div><span class="eyebrow">TODAY EXECUTION · ${esc(today)}</span><h2>今日执行</h2><p>${activeBlock?`当前时间块：${esc(activeBlock.title)}`:nextBlock?`下一步：${esc(nextBlock.title)}`:'今天暂无进行中的时间块'}</p></div><div class="execution-stats"><span><b>${blocks.length}</b><small>时间块</small></span><span><b>${done}/${blocks.length||0}</b><small>已完成</small></span><span><b>${overdue.length}</b><small>逾期未完成</small></span><span><b>${blocked.length}</b><small>阻塞</small></span></div></div><div class="execution-grid"><section class="card execution-card"><div class="sec-title" style="margin-top:0">今日时间块</div>${blockRows}</section><section class="card execution-card"><div class="sec-title" style="margin-top:0">行动队列</div>${taskRows}</section></div><section class="card execution-alert"><div><b>执行异常</b><span>系统已根据截止日期和任务状态整理，具体处理建议可由 AI 分析后提供。</span></div><div class="execution-alert-items">${alertItems.length?alertItems.join(''):'<span>当前没有需要立即处理的异常</span>'}</div></section></div>`;
}
